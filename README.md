...
不要把真实 API key、真实 `JWT_SECRET_KEY`、客户数据或生产数据库密码提交到 GitHub。仓库里的 `.env` 只应该保留空值、demo 值或明确占位值。
...
1. 打开 `.env`，按本手册说明替换 `JWT_SECRET_KEY` 和需要使用的 API key。
2. 安装依赖。
3. 启动 uvicorn。
